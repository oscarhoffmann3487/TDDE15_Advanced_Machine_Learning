...
```         
>>>>>>> 9d006c8b05e6345c59de8efaac8face50706fb53
```
...
marginal_log <- matrix(0, ncol = length(sigmaF), nrow = length(ell))
for (i in 1:length(sigmaF)){
  for (j in 1:length(ell)){
    res = posteriorGP(time, temps, time, sigmaNoise, k = SEKernel(ell = ell[j], sigmaF = sigmaF[i]))
 marginal_log[i,j] = res$log_likelihood
  }
}
marginal_log
